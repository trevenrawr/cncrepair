 ...
 */
 ...
 // Logging functions!
 function qb_log_message($message) {
	$fp = fopen('logs/qblog'.date('m-d-y').'.txt', 'a+');
	fwrite($fp, date('H:i:s-->   ').$message."\n\n");
	fclose($fp);
	if ( '0771' !== fileperms('logs/qblog'.date('m-d-y').'.txt'))
	{
		chmod('logs/qblog'.date('m-d-y').'.txt', 0771);
	}
}
